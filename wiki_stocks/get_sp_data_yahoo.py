        ...
        try:
            for item in json_result['timeseries']['result']:
                try:
                    ...
                    # if finance_type == 'trailingCommonStockIssuance':
                    #     print(item)
                    if finance_type in item:
                        for record in item[finance_type]:
                            try:
                                if record is not None:
                                    ...
                                    # print(period)
                                    asofdate = datetime.datetime.strptime('1999-01-01', "%Y-%m-%d").date()
                                    if 'asOfDate' in record:
                                        asofdate = datetime.datetime.strptime(record['asOfDate'], "%Y-%m-%d").date()
                                    # print(asofdate)
                                    value = 'NONE'
                                ...
                                print("cash_flow_annual Unknown Exception:", e)
