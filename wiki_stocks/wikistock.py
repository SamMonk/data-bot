...
class SPData(Base):
    ...
    #id = Column(Integer, primary_key=True)
    sp_ticker = Column('sp_ticker',String(8))
    ...
    __table_args__ = (
        UniqueConstraint('sp_ticker', 'type', 'date', 'value', name='_data_unique'),
        PrimaryKeyConstraint('sp_ticker', 'type', 'date', 'value'),
    )

    def __init__(self, sp_ticker, financial_type, record_date, record_value):
